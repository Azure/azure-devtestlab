...
Param(
    ...
    # Enter the name of the VM service. The service name is optional and it usually matches the DNS entry in Cloud Service.
    [string]
    $VMServiceName = '',
...
function Get-AzureDtlVirtualMachine
{
    ...
    $vm = Get-AzureVM -Name "$Name" -ServiceName "$ServiceName" | Select-Object -First 1
    if (-not $vm)
    {
...
function Stop-AzureDtlVirtualMachine
{
    ...
    $classicVm = Get-AzureVM | Where-Object { $_.Name -eq $VM.Name }
...
try
{
    ...
    $vm = Get-AzureDtlVirtualMachine -Name "$VMName" -ServiceName "$VMServiceName"
    Write-Host $Done
