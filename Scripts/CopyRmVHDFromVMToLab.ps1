...
function Get-AzureDtlVirtualMachine
{
    ...
    $vm = Get-AzureRmVM -Name "$Name" -ResourceGroupName "$ResourceGroupName" | Select-Object -First 1
    if (-not $vm)
    {
...
function Get-AzureDtlVirtualMachineCopyContext
{
    ...
    $properties = (Get-AzureRMResource -ResourceType $VM.Type -ResourceName $VM.Name -ResourceGroupName $VM.ResourceGroupName).Properties
    ...
    elseif ($properties.storageProfile.osDisk.vhd.uri)
    {
        ...
        $vmStorageAccount = Get-AzureRmResource -Name $vmCopyContext.StorageAccountName -ResourceType 'Microsoft.Storage/storageAccounts'
...
function Copy-AzureDtlVirtualMachineVhd
{
    ...
    New-AzureStorageContainer -Name $destContainerName -Context $destContext -Permission Off -ErrorAction SilentlyContinue | Out-Null
    Write-Host $Done

    if ($CopyContext.IsVMDiskManaged)
    {
        Write-Host "  copying managed disk ... " -NoNewline
        $copyHandle = Start-AzureStorageBlobCopy -AbsoluteUri $CopyContext.VMSourceUri -DestContainer $destContainerName -DestBlob $CopyContext.VHDFileName -DestContext $destContext
    }
    else
    {
        ...
        $copyHandle = Start-AzureStorageBlobCopy -AbsoluteUri $CopyContext.VMSourceUri -Context $srcContext -DestContainer $destContainerName -DestBlob $CopyContext.VHDFileName -DestContext $destContext
    }
...
try
{
    ...
    $vm = Get-AzureDtlVirtualMachine -Name "$VMName" -ResourceGroupName "$VMResourceGroupName"
    Write-Host $Done

    Write-Host "Stopping source virtual machine '$VMName' ... " -NoNewline
    Stop-AzureDtlVirtualMachine -VM $vm
    Write-Host $Done

    Write-Host 'Preparing copy context ... ' -NoNewline
    $vmCopyContext = Get-AzureDtlVirtualMachineCopyContext -VM $vm -SignatureExpire $SignatureExpire
    $copyContext = @{
