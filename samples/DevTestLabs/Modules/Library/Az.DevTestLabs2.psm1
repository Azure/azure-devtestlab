...
function MyGetResource {
  ...
  if($ResourceGroupName -and (-not $ResourceGroupName.Contains("*"))) { # Proper RG
    if($Name -and (-not $Name.Contains("*"))) { # Proper RG, Proper Name
      ...
      Get-AzureRmResource -ExpandProperties -resourcetype $ResourceType -ResourceGroupName $ResourceGroupName -EA SilentlyContinue | Where-Object { $_.ResourceName -like $Name}
    }
  } else { # Wild RG forces client side query anyhow
    Get-AzureRmResource -ExpandProperties -resourcetype $ResourceType -EA SilentlyContinue | Where-Object { $_.ResourceName -like $Name -and $_.ResourceGroupName -like $ResourceGroupName}
  }
}
